...
@CrossOrigin(origins = "*")
public class UserResource implements IBaseController<User> {
    @Autowired
    ...
    @Override
    @PostMapping
    public ResponseEntity<ResponseFormatter> create(@RequestBody User entity) {
        try {
    ...
    @Override
    @PutMapping("/{id}")
    public ResponseEntity<ResponseFormatter> update(@PathVariable UUID id, @RequestBody User entity) {
        try {
